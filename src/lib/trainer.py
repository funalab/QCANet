    ...
    def NSNTraining(self, trainIdx, testIdx, xlist, ylist, kc):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            if bestRecall <= teseval['Recall']:
                ...
                # Save Model
                model_name = 'NSN_Recall_npz_p' + str(self.patchsize) + '_k' + str(kc+1) + '.model'
                serializers.save_npz(self.opbase + '/' + model_name, self.model)
            if bestPrecision <= teseval['Precision']:
                bestPrecision = teseval['Precision']
                # Save Model
                model_name = 'NSN_Precision_npz_p' + str(self.patchsize) + '_k' + str(kc+1) + '.model'
                serializers.save_npz(self.opbase + '/' + model_name, self.model)
            ...
            if bestIoU <= teseval['IoU']:
                ...
                bestEpoch = epoch
                # Save Model
                model_name = 'NSN_IoU_npz_p' + str(self.patchsize) + '_k' + str(kc+1) + '.model'
                serializers.save_npz(self.opbase + '/' + model_name, self.model)
    ...
    def NDNTraining(self, trainIdx, testIdx, xlist, ylist, kc):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            if bestRecall <= teseval['Recall']:
                bestRecall = teseval['Recall']
                # Save Model
                model_name = 'NDN_Recall_npz_p' + str(self.patchsize) + '_k' + str(kc+1) + '.model'
                serializers.save_npz(self.opbase + '/' + model_name, self.model)
            if bestPrecision <= teseval['Precision']:
                bestPrecision = teseval['Precision']
                # Save Model
                smodelfile = 'NDN_Precision_npz_p' + str(self.patchsize) + '_k' + str(kc+1) + '.model'
                serializers.save_npz(self.opbase + '/' + model_name, self.model)
            if bestFmeasure <= teseval['F-measure']:
                bestFmeasure = teseval['F-measure']
                bestEpoch = epoch
                # Save Model
                model_name = 'NDN_Fmeasure_npz_p' + str(self.patchsize) + '_k' + str(kc+1) + '.model'
                serializers.save_npz(self.opbase + '/' + model_name, self.model)
            if bestIoU <= teseval['IoU']:
                bestIoU = teseval['IoU']
